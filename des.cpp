...
const static char InitialPermutationTable[] =	   {58, 50, 42, 34, 26, 18, 10, 2,
...
const static char InitialKeyPermutation[] = {57, 49,  41, 33,  25,  17,  9,
								 1, 58,  50, 42,  34,  26, 18,
								10,  2,  59, 51,  43,  35, 27,
								19, 11,   3, 60,  52,  44, 36,
...
const static char shiftTable[] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};

const static char SubKeyPermutationTable[] =    {14, 17, 11, 24,  1,  5,
								 3, 28, 15,  6, 21, 10,
								23, 19, 12,  4, 26,  8,
								16,  7, 27, 20, 13,  2,
								41, 52, 31, 37, 47, 55,
...
const static char MessageExpansionTable[] =  {32,  1,  2,  3,  4,  5,
							 4,  5,  6,  7,  8,  9,
							 8,  9, 10, 11, 12, 13,
							12, 13, 14, 15, 16, 17,
							16, 17, 18, 19, 20, 21,
...
const static char sBoxPermutationTable[8][4][16] = {
	{
...
const static char RightSubMessagePermutation[] =  {16,  7, 20, 21, 29, 12, 28, 17,
                                        1,  15, 23, 26, 5,  18, 31, 10,
                                        2,  8,  24, 14, 32, 27,  3,  9,
                                        19, 13, 30,  6, 22, 11,  4, 25};

const static char InversePermutationTable[] =  {40,  8, 48, 16, 56, 24, 64, 32,
...
void Transform(bool *subKey, bool *mainKey,const char *changeTable, int size);
...
void tranferToBit(bool *out, const char *in, int size) {
    for(int i=0;i<size;i++) {
        out[size-1-i] = (in[i/8] >> (i % 8)) & 0x1;
    }
}
...
void S_function(bool out[32], bool in[48]) {
    ...
    for(j=0;j<8;j++,in+=6,out+=4) {
        ...
        tranferToBit(out, &sBoxPermutationTable[j][m][n], 4);
    }
}
